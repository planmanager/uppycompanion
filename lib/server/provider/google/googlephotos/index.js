    ...
    async list(options) {
        return withGoogleErrorHandling(GooglePhotos.oauthProvider, 'provider.photos.list.error', async () => {
            ...
            async function fetchAlbums() {
                if (!isRoot)
                    return []; // albums are only in the root
                return paginate((pageToken) => client.get('albums', { searchParams: { pageToken, pageSize: 50 }, responseType: 'json' }).json(), (response) => response.albums ?? []);
            }
            async function fetchSharedAlbums() {
                if (!isRoot)
                    return []; // albums are only in the root
                return paginate((pageToken) => client.get('sharedAlbums', { searchParams: { pageToken, pageSize: 50 }, responseType: 'json' }).json(), (response) => response.sharedAlbums ?? []);
            }
            async function fetchMediaItems() {
                if (isRoot)
                    return { mediaItems: [] }; // no images in root (album list only)
            ...
            if (nextPageToken)
                newSp.set('cursor', nextPageToken);
            const iconSize = 64;
            const thumbSize = 300;
